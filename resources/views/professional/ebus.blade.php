    ...
    <div class="col-md-6 lighter-grey-bg">
      ...
      <div class="row">
        <div class="col-md-6 text-left event-items-category">
          <p><em>@if(isset($course->type)){{$course->type}}@endif</em></p>
        ...
        <div class="col-md-6 text-right">
          @if(isset($course->programmeFile))  
            <a href="{{$course->programmeFile->fileUrl}}" target="_blank" type="button" class="btn btn-light-primary text-left">
              ...
              @if(isset($course->programmeButtonText)){{$course->programmeButtonText}}@else Course Programme @endif
            </a>
          @endif
          @if(isset($course->programmeNotice))
          <p class="text-danger">{{$course->programmeNotice}}</p>
          @endif
        </div>
      </div>
      ...
      <h5><a href="{{$course->disclosureFile->fileUrl}}" target="_blank">Faculty disclosures</a></h5>
      @endif
          ...
          <div id="venue" class="tab-pane active cont">
            ...
            <div class="ers-scroller nano scrollable" style="height:350px;">
                ...
                <ul class="list-group">
                  ...
                  @endif
                  @if(isset($course->ebusVenues))
                  <li class="list-group-item"><a data-toggle="modal" data-target="#md-venue_accommodation" type="button" class="">Venue and Dates</a></li>
                  @endif
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="text-left">
        <div class="row">
          @foreach ($course->ebusVenues as $venue)
              <div class="col-md-6">
                <p>
                  @if(isset($venue['url']))
                  ...
                  @endif
                  <br/>
                  @if(isset($venue['streetAddress']))
                  {{$venue['streetAddress']}}<br>
                  @endif
                  @if(isset($venue['streetAddress2']))
                  {{$venue['streetAddress2']}}<br>
                  @endif
                  @if(isset($venue['streetAddress3']))
                  {{$venue['streetAddress3']}}<br>
                  @endif
                  @if(isset($venue['zip'])){{$venue['zip']}} {{$venue['city']}}<br>@endif
                  @if(isset($venue['zip'])){{$venue['country']}}@endif
                </p>
                
                  @if(isset($venue['info'])){!!$venue['info']!!}@endif
